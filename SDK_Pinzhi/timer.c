 ...
 *
 * 例如设置100ms的中断周期:
 * - 重装载值: finalValue = 3750
 * -   分频: ftmPrescaler = 128
 * 
 * 3750(重载值)/(48000000hz(主频)/128(分频)) = 1/10秒 = 10ms
 */
static void Timer0_ISR_10ms(void)
{
    counter_tmr0++;
    
    if(counter_tmr0 >= 10000U)
    {
        counter_tmr0 = 0;
    }
...
void Timer_Init(void)
{
    ...
    INT_SYS_InstallHandler(FTM0_Ovf_Reload_IRQn, &Timer0_ISR_10ms, (isr_t*) 0U);
